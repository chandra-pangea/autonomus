---
name: "OPSX: Tasks JSON"
description: "Output all OpenSpec tasks as structured JSON with status (todo/in_progress/completed)"
category: Workflow
tags: [workflow, tasks, json, status, experimental]
---

Output all OpenSpec tasks as structured JSON.

**Input**: Optionally specify a change name to filter (e.g., `/opsx:tasks-json add-auth`). If omitted, shows tasks from ALL active changes.
...
Always output ONLY the JSON — no surrounding explanation, no markdown fences, no commentary before or after. Just raw JSON so it can be piped or copied directly.
