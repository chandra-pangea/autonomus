---
name: openspec-run
description: Run a single task by ID from an OpenSpec change. Auto-archives when all tasks are complete. Use when the user wants to execute one specific task at a time.
license: MIT
...
This is the "just do it" command — picks up where you left off, implements one task, and archives the change when all tasks are done. No extra commands needed.

**Input**: Specify a task ID and optionally a change name.

Examples:
- `@run 1.1` — Run task 1.1 (auto-selects change if only one)
- `@run 2.3 add-auth` — Run task 2.3 from change "add-auth"
- `@run next` — Run the next pending task
- `@run next add-auth` — Run the next pending task from "add-auth"
...
1. **Select the change**
   ...
   - If multiple exist, run `openspec list --json` and prompt the user to pick
   - If no changes exist: "No active changes. Create one first." — stop
